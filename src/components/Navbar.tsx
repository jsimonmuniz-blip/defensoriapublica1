...
import logoUrl from "@/assets/logo-idpnl-nuevo.png";
...
const ADVOCATUS_URL = "https://advocatus.idpnl.gob.mx/";
    ...
    <header className="sticky top-0 z-40 w-full border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
      <div className="mx-auto flex h-24 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link to="/" className="flex shrink-0 items-center" onClick={() => setOpen(false)}>
          <img src={logoUrl} alt="Instituto de Defensoría Pública de Nuevo León" className="h-16 w-auto sm:h-20" />
        </Link>
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          ...
          <a
            ...
            className="rounded-md px-3 py-2 text-sm font-semibold text-primary transition-colors hover:bg-accent"
          >
            ADVOCATUS
          </a>
        </nav>
